...
template<typename F, typename K>
 bool areEqual(const K &reference, const  F & q, const double precision = 1e-6) {
...
void TestAxisAngle(){
    ...
    {   //conversion to matrix:
        ...
        if(!areEqual(std::array<double, 9>{1., 0., 0., 0. , 0.1542515 , 0.9880316, 0., -0.9880316, 0.1542515}, *converted) ){
            numErrors ++;
    ...
    {   //conversion to quaternion:
        ...
        if(!areEqual(std::array<double, 4>{-0.7596879, 0.6502878, 0., 0.}, *converted) ){
            numErrors ++;
...
void TestMatrix(){
    ...
    {
        Matrix3<double> m({1., 2., 3., 4., 5., 6., 7., 8., 9.});
        if(!areEqual(std::array<double, 9>{1., 2., 3., 4., 5., 6., 7., 8., 9.}, m) ){
            numErrors ++;
    ...
    // quaternion: [ x = 0.6502878, y = 0,  z = 0, w = -0.7596879 ]
        ...
        if(q){
            auto val = q.value();
            if(!areEqual(std::array<double, 4>{-0.7596879, 0.6502878, 0., 0.}, val )){
                numErrors++;
    ...
    {
        Matrix3<double> m({1., 0., 0., 0. , 0.1542515 , 0.9880316, 0., -0.9880316, 0.1542515});
        std::array<double,3> vec{0., 1., 0.}; //v || y, expected : (0, cos(30), sin(30))
        std::optional<std::array<double,3>> result = m*vec;
        if(result){
            auto value = result.value();
            double s = std::sin(30);
            double c = std::cos(30); 
            if(result and !areEqual(std::array<double, 3>{0, c, s}, value )){
                numErrors++;
    ...
    //Test matrix multiplication
    {
        ...
        if(!areEqual(std::array<double, 9>{13. , 17. ,  22.4, 34.,  47.,  56., 55.,  77., 89.6}, c)){
            numErrors++;
...
void TestQuaternion() {
    ...
    {   //Rotation by 30 degs around x axis: (1, 0, 0)
        ...
        if(!areEqual(std::array<double, 4>{-0.7596879, 0.6502878, 0., 0.}, q)){
            numErrors++;
...

    {
        ...
        if(!areEqual(std::array<double, 4>{0.5, 1.25, 1.5, 0.25}, res)){
            numErrors++;
    ...
    // r = [0., 1., 0.]
        ...
        std::array<double,3> r {0., 1., 0.};
        std::optional<std::array<double,3>> result = rotateByQuaternion(q, r); 
        double s = std::sin(30);
        double c = std::cos(30);
        if(result){
            if(!areEqual(std::array<double, 3>{0, c, s}, *result )){
                numErrors++;
    ...
    // quaternion: [ x = 0.6502878, y = 0,  z = 0, w = -0.7596879 ]
        ...
        if(!areEqual(std::array<double, 9>{1., 0., 0., 0. , 0.1542515 , 0.9880316, 0., -0.9880316, 0.1542515}, m )){
            numErrors++;
